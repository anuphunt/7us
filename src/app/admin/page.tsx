  ...
  return (
    <main className="min-h-screen p-6">
      ...
      <p className="mt-2 text-sm text-neutral-600">
        Dashboard scaffolding (override requests, timesheets, schedules, tasks).
      </p>
      ...
      <section className="mt-8 max-w-lg rounded border p-4">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <button
            className="mt-2 rounded bg-black px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
            onClick={onSave}
        ...
        <div className="mt-4 grid gap-3">
          <label className="grid gap-1 text-sm">
            ...
            <input
              ...
              maxLength={2}
              value={newUserId}
              onChange={(e) => setNewUserId(e.target.value.replace(/\D/g, '').slice(0, 2))}
              placeholder="00"
            />
          </label>
          <label className="grid gap-1 text-sm">
          ...
          {userStatus === 'saved' && (
            <div className="text-sm text-emerald-600">
              User created (UI stub — hook up backend next).
            </div>
          )}
          {userStatus === 'error' && (
